...
public class NotificationReceiverTest {
    ...
    public void testOnReceive_showNotificationIntent() {
        ...
        }).when(notificationManager).notify(Mockito.anyString(), Mockito.anyInt(), Mockito.any());
    ...
    public void testOnReceive_openNotificationLaunchIntentNotNull() {
        ...
        PowerMockito.when(packageManager.getLaunchIntentForPackage(Mockito.anyString())).thenReturn(launchIntent);
        PowerMockito.doAnswer(new Answer<Object>() {
            @Override
            public Object answer(InvocationOnMock invocation) throws Throwable {
                Intent launchIntentWithExtras = (Intent) invocation.getArguments()[0];
                boolean isOpenFromNotification = launchIntentWithExtras
                        .getBooleanExtra("EMS_OPEN_FROM_NOTIFICATION", false);
                Assert.assertTrue(isOpenFromNotification);
                return null;
            }
        }).when(context).startActivity(Mockito.any());
