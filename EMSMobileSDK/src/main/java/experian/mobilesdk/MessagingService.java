...
/**
 * Firebase Messaging is the Google API for handling Push Notifications on Android. This class
 * extends the base Messaging Services.
 */
public class MessagingService extends FirebaseMessagingService {
  ...
  /**
   * This event fires when a Push Notification is received from Firebase and the application is
  ...
  public void onMessageReceived(RemoteMessage remoteMessage) {
    ...
    if (remoteMessage.getData().size() > 0) {
      ...
      Intent intent =
          new Intent(getApplicationContext().getPackageName() + EMSIntents.EMS_PUSH_RECEIVED);
      ...
      intent =
          new Intent(getApplicationContext().getPackageName() + EMSIntents.EMS_SHOW_NOTIFICATION);
      intent.setPackage(getApplicationContext().getPackageName());
      intent.putExtra("data", remoteMessage);
      sendBroadcast(intent);
    }
